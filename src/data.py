...
class Data: 
    ...
    def get_random_item(self,
        ...
        ----------
        x : Iterable
            optional (x OR rng), default = [] - List of default type
        cnt: int
            optional, default = 1 - the count of how many random item
        rng : Tuple
            optional (x OR rng) - select a random number from a range where [0]
                is start, [1] is stop, [3] is step

        seed : int
            int value 
            optional, default = 0, used to create a none-random array based on
                local host clock
        ...
        if all([isinstance(x, Iterable),
            ...
            if len(x) > 0 and rng[1] > 0:
                err_msg = f'x ({len(x)}) AND rng ({rng}),\
                        both cannot be calculated'
                raise InvalidParamValue(err_msg)
            elif cnt < 1:
                err_msg = f'cnt ({len(cnt)}) must be greater \
                        than or equal to 1'
                raise InvalidParamValue(err_msg)            
            else: 
                res_array = []
                ...
                if len(x) > 0:
                    ...
                    if len(rng) == 3:
                        for i in range(cnt):
                            res_array.append(random.randrange(rng[0],
                                                              rng[1], rng[2]))
                    elif len(rng) == 2:
                        for i in range(cnt):
    ...
    def get_closest(self, lst: list, x_arr: int | float):
        """
        Find the value in the list that is closest to the x_arr value /
            passed as an argument
        
        Parameters
        ----------
        lst: list
            mandatory - a list that contains the values used to search for /
                the closest value
    ...
    def make_discrete_data(self,
                           N: int = 0, mu: int | float = 0,
                           sigma: float = 0,
                           seed: int = 246810):
        """
        create random integers using numpy.random.generator.integers() \
                and return as a list determined by size N, mu, sigma \
                passed as parameters  

                       
        calculate list, min_val, max_val, mu, sigma and count of the  
    ...
    def convert_to_std_norm(self, x: np.array):
        """
        ...
        if isinstance(x, np.ndarray):
            ...
            err_msg = 'x - Invalid Argument passed'
            raise InvalidParamEntry(err_msg)

    def get_normal_dist(self,
                        x_arr,
                        mu: int | float = 0.0,
                        sigma: int | float = 10.0):
        """
        ...
        ----------
        x_arr : list
            mandatory - A single value, or array that is the basis for /
                the probability 
        mu : int | float
    ...
    def get_poisson_pmf_distro(self, mu: int | float, loc: int | float, n: int):
        ...
        """
        create an array of pmf values based on poisson location,
            distribution mean and distribution size
        An x-axis array is created first with the range of N
        scipy.stats.poisson.pmf() is used to create the y-axis
        ...
        Returns
        -------
        x: mandatory - an numpy array of 0 to N values, y: a scipy pmf
            array of probabilities based on x
        """
    ...
    def get_poisson_cdf_value(self, k: int):
        """
        Calculates the cumulative density function of a specific
            range given as a parameter.
    ...
    def get_poisson_ppf_value(self, k: int = 0, mu: int | float = 0):
        """
        the percent point function takes the probability value and
    ...
    def get_poisson_prob(self, info: Dict, std_out: str = "N"):
        ...
        if isinstance(info, Dict):
            ...
            if std_out == "N":
                ...
                msg = msg + "x = %s\\\\~\\\\"
                msg = msg + "P(Y) = \\dfrac{\\lambda^y e^{- \\lambda}}{y!} \
                        = \\dfrac{%s^%s %s^{- %s}}{%s!} = \
                            \\dfrac{%s}{%s} = %s\\\\~\\\\"
                msg = msg + "\\color{dodgerblue}\\textbf{There was a %s \
                        percent chance of observing exactly %s~%s in %s %s}"
    ...
    def get_mean(self, data_set: List):
        ...
        """   
        if isinstance(data_set, list | np.ndarray):
    ...
    def convert_to_std_dev(self, x: int | float,
                           mu: int | float, sigma: int | float,
                           std_out: str = "N"):
        """
        ...
        if all([isinstance(x, int | float),
            ...
            if std_out.upper() == "Y":
                from IPython.display import display, Math
                msg = '\\displaystyle \\text{x %s is %s standard \
        ...
        if all([(isinstance(x, list)), (isinstance(std_dev, int | float)), (isinstance(auc, int | float)), 
            ...
            if (std_dev == 0 and auc != 0) or (std_dev != 0 and auc == 0): 
                ...
                # Use area under curve, not std dev
                    ...
                    res = x.index(self.get_closest(x,
                                                   self.get_x_axis_value(z=self.get_z_critical_value(x=auc),
                                                                         sigma=sigma, mu=mu)))
                    return res
            else:
    ...
    def get_sample_proportion_std_err_with_fpc(self, p_hat: int | float, n: int | float, N: int | float):
        """
        Calculate standard error for the sample proportion based on the parameter n,i \
                p_hat and N with finite correction factor
    ...
    def get_matched_pair_tscore(self, d_bar: int | float, mu_d: int | float, s_d: int | float, n: int) -> Tuple:
       ...
       Calculate test statistic for matched pair sample with small sample size 
        ...
        """
        if all([isinstance(d_bar, int | float),
                isinstance(mu_d, int | float),
                isinstance(s_d, int | float),
                isinstance(n, int)]):
    ...
    def get_geometric_probability(self, info: Dict, std_out: str = "N"):
        ...
        if isinstance(info, dict):
            ...
            if std_out == "N":
                ...
                msg = msg + "P(S = n) = P(1 - P)^{n - 1} = %s(1 - %s)^{%s - 1} = %s \
                        \\cdot %s^%s = %s \\cdot %s = %s\\\\~\\\\"
        ...
        if all([(isinstance(tail, str)), (isinstance(q, float)), (isinstance(df, int))]):
            if tail.lower() in ['lower', 'upper', 'two']:
                if tail.lower() == 'upper':
                    ...
                    if std_out == 'Y':
                        from IPython.display import display, Math 
                        msg = '\\displaystyle \\text{The %s tail critical value for a probabilility \
                                of %s with %s degrees of freedom is %s}'
    ...
    def get_z_score(self,
        ...
        z-score -> float
        """            
        if all([isinstance(mu, int | float),
                isinstance(observed_value, int | float),
                isinstance(std_dev, int | float),
                isinstance(n, int)]):
            import numpy as np
    ...
    def get_rejection_regions(self, x: list = [], tail: str = '', alpha: float = 0,
                              observed_value: int | float = 0, sigma: int | float = 0):
        ...
        if all([(isinstance(x,
            ...
            if tail.lower() in ["lower", "upper", "two"]:
                if tail == "upper":
                    ...
                    upper_critical_value_index = (self.get_x_axis_index(x=x,
                                                                        auc=(1 - alpha),
                                                                        mu=observed_value,
                                                                        sigma=sigma) + 1)
                    ...
                    lower_critical_value_index = (self.get_x_axis_index(x=x,
                                                                        auc=alpha,
                                                                        mu=observed_value,
                                                                        sigma=sigma) + 1)
    ...
    def is_in_rejection_region(self, tail: str = '',
                               test_statistic: int | float = 0,
                               critical_value: int | float = 0,
                               mu: int | float = 0):
        ...
        if all([(isinstance(tail, str)),
                isinstance(test_statistic, int | float),
                isinstance(critical_value, int | float),
                (isinstance(mu, int | float))]):
            if tail.lower() in ["lower", "upper", "two"]:
    ...
    def check_proportion_distro_for_normality(self,
                                              p1: float,
                                              p2: float,
                                              n1: int,
                                              n2: int) -> bool:
    ...
    def get_test_statistic_for_diff_of_props(self,
                                             p_diff: int | float,
                                             phat: float,
                                             p1: float,
                                             p2: float,
    ...
    def get_y_intercept(self, x: List, y: List, m: int | float): 
        ...
        """ 
        if all([isinstance(x, List | np.ndarray), isinstance(y, List | np.ndarray), isinstance(m, int | float)]):
            if len(x) > 3 and len(y) > 3:
                n = len(x)
                sigma_x = sum(x)
                sigma_y = sum(y)
            ...
            err_msg = f'x {type(x)} and y {type(y)} must be of the type list or numpy.ndarray and m {type(m)} \
            must be of the tye int or float'
            raise InvalidParamEntry(err_msg)  
    ...
    def get_corr_coeff(self, x: List | np.ndarray, y : List | np.ndarray):
        """
        ...
        """ 
        if all([isinstance(x, List | np.ndarray), isinstance(y, List | np.ndarray)]): 
            n = len(x)
            sum_x = sum(x)
            sum_y = sum(y)
            xy = [x[i] * y[i] for i in range(len(x))]
            ...
            sum_x_sqr = sum(x_sqr)
            y_sqr = [y[i]**2 for i in range(len(y))]
            sum_y_sqr = sum(y_sqr)
            num = (n*sum_xy) - (sum_x * sum_y)
            denom = np.sqrt(((n * sum_x_sqr) - sum_x**2) * (((n * sum_y_sqr) - sum_y**2)))
            return num / denom 
        else:
            err_msg = 'x and y must be of the type list'
            raise InvalidParamEntry(err_msg)   
            
    def calc_gradient_descent(self, x: List, y: List, learning_rate: int | float, epochs: int, display: bool = False):
        """
        ...
        learning_rate: int | float
            mandatory - the learning rate 
        epochs: int
            mandatory - the number of iterations 
        display: bool
            optional - display the gradient descent output 
        ...
        if all([isinstance(x, list),
            ...
            for i in range(epochs):
                ...
                m = m - learning_rate * (2/n) * sum(x * (y_pred - y))
                if display:
                    print(f'Epoch {i} => m: {m}, cost: {cost}')
            return m
        else:
            err_msg = 'x and y must be of the type list'
            raise InvalidParamEntry(err_msg)
    ...
    def chart_regression(self, x: List, y: List):
        if isinstance(x, np.ndarray):
            x_tbl = copy.copy(x).tolist()
            y_tbl = copy.copy(y).tolist()
        else:
            x_tbl = copy.copy(x)
            y_tbl = copy.copy(y)
        mu_x = np.mean(x)
    ...
    def get_ols(self, x1: pd.DataFrame | pd.Series, y1: pd.Series, display_ols: bool = False):
        ...
        pvalue_labels = [label + '_pvalue' for label in tmp_labels]
        stderr_labels = [label + '_stderr' for label in tmp_labels]
        tstatistic_labels = [label + '_tstatistic' for label in tmp_labels]
    ...
    def get_regressor(self,
                      ...
                      seed: float = 1016,
                      intercept: bool = True):
        """create a sklearn linear regressor 

        Args:
            x (np.ndarray): an ndarray of the predictor variables 
            y (list | np.ndarray): a ndarray of the target
            test_size (float): percentage size of the test set
            seed (float, optional): unique . Defaults to 1016.
            intercept (bool, optional): _description_. Defaults to True.

        Raises:
            InvalidParamEntry: _description_
        ...
        if all([isinstance(x,
                ...
                isinstance(seed, int), isinstance(intercept, bool)]):        
            ...
            # create f_regression for f-statistics, p-values
            
            res = Regressor(
                r_squared=regressor.score(X_test, y_test),
                adj_r_squared=self.get_adj_rsquared(regressor.score(X_test, y_test),
                                                    X_test.shape[0],
                                                    X_test.shape[1]), 
                coefs=regressor.coef_,
                intercept=regressor.intercept__,
                regressor=regressor, 
                f_statistics=[], # get f-statistics from f_regression
                p_values=[]) # get p-values from f_regression 
    ...
    def get_adj_rsquared(self, r_squared: int | float, n : int, k: int ):
        """_summary_

        Args:
            r_squared (int | float): coefficient of determination 
            n (int): the number of observations (rows)
            k (int): the number of predictor values (independent variables )
        """
        if all([isinstance(r_squared, float), isinstance(n, int), isinstance(k, int)]):
...
def x_is_y_pct_of_what(x : int | float, y : int | float):
    """_summary_
